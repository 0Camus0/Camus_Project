#ifndef T1000_SCENEPROPS_H
#define T1000_SCENEPROPS_H
...
struct SSAOFilter {
	...
	t1000::Texture* NoiseTex;
	std::vector<XVECTOR3> vSSAOKernel;
	void InitTexture();
	void Update();
};
struct SceneProps{
	SceneProps() : ActiveCamera(0) , ActiveLights(1), ActiveLightCamera(0), ActiveGaussKernel(0), ToogleShadow(1), ToogleSSAO(1), Exposure(0.3f) , BloomFactor(0.35f){}
