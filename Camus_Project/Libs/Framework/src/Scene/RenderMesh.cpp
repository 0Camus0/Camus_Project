...
namespace t1000 {
  ...
  void RenderMesh::Load(char *filename)
  {
    //xFile = pApp->resourceManager.Load(filename);
	  xFile = new xF::XDataBase;
	  xFile->LoadXFile(filename);
  }
  ...
  void RenderMesh::Create() {
    ...
    for (std::size_t i = 0; i < xFile->MeshInfo.size(); i++) {
      ...
      it_MeshInfo->CB = (t1000::ConstantBuffer*)T8Device->CreateBuffer(T_BUFFER_TYPE::CONSTANT, bdesc);
      ...
      for (int j = 0; j < NumMaterials; j++) {
        ...
        for (int k = 0; k < NumFaceIndices; k++) {
          if (pActual->MaterialList.FaceIndices[k] == (unsigned int)j) {
        ...
        bdesc.usage = T_BUFFER_USAGE::DEFAULT;
        it_subsetinfo->IB = (t1000::IndexBuffer*)T8Device->CreateBuffer(T_BUFFER_TYPE::INDEX, bdesc, tmpIndexex);
	  ...
	  t1000::BufferDesc buffdesc;
      buffdesc.byteWidth = pActual->NumVertices*it->VertexSize;
      buffdesc.usage = T_BUFFER_USAGE::DEFAULT;
      it_MeshInfo->VB = (t1000::VertexBuffer*)T8Device->CreateBuffer(T_BUFFER_TYPE::VERTEX, buffdesc, &it->pData[0]);
      ...
      buffdesc.usage = T_BUFFER_USAGE::DEFAULT;
      it_MeshInfo->IB = (t1000::IndexBuffer*)T8Device->CreateBuffer(T_BUFFER_TYPE::INDEX, buffdesc, &pActual->Triangles[0]);
    }
  ...
  void RenderMesh::GatherInfo() {
    ...
    if (g_pBaseDriver->m_currentAPI == T_GRAPHICS_API::_OPENGL) {
      vsSourceP = file2string("VS_Mesh.glsl");
      fsSourceP = file2string("FS_Mesh.glsl");
    }
    else {
      vsSourceP = file2string("VS_Mesh.hlsl");
      fsSourceP = file2string("FS_Mesh.hlsl");
    }
  ...
  //      xSubsetInfo *subinfo = &it->Subsets[j];
        ...
        for (unsigned int k = 0; k < material->EffectInstance.pDefaults.size(); k++) {
          ...
          if (mDef->Type == xF::xEFFECTENUM::STDX_STRINGS) {
            if (mDef->NameParam == "diffuseMap") {
              CurrSig |= T_Signature::DIFFUSE_MAP;
            }

            if (mDef->NameParam == "specularMap") {
              CurrSig |= T_Signature::SPECULAR_MAP;
            }

            if (mDef->NameParam == "glossMap") {
              CurrSig |= T_Signature::GLOSS_MAP;
            }

            if (mDef->NameParam == "normalMap") {
              CurrSig |= T_Signature::NORMAL_MAP;
            }

            if (mDef->NameParam == "heightMap") {//
              CurrSig |= T_Signature::HEIGHT_MAP;
            }
          }
          ...
          if (mDef->Type == xF::xEFFECTENUM::STDX_DWORDS) {
            if (mDef->NameParam == "NoLighting") {
              if (mDef->CaseDWORD == 1) {
                CurrSig |= T_Signature::USE_NO_LIGHT;
              }
            }
			if (mDef->NameParam == "bUseFresnel") {
				if (mDef->CaseDWORD == 1) {
					CurrSig |= T_Signature::USE_FRESNEL;
				}
			}
          }
        }
		
		if (CurrSig&T_Signature::USE_NO_LIGHT) {
			stmp.MatID = 0;
		}
		else if (!(CurrSig&T_Signature::NORMAL_MAP) && !(CurrSig&T_Signature::USE_FRESNEL)) {
			stmp.MatID = 1;
		}
		else if (CurrSig&T_Signature::NORMAL_MAP && !(CurrSig&T_Signature::USE_FRESNEL)) {
			stmp.MatID = 2;
		...
		if (CurrSig&T_Signature::USE_FRESNEL)
			CurrSig ^= T_Signature::USE_FRESNEL;
        ...
        CurrSig |= T_Signature::FORWARD_PASS;
        g_pBaseDriver->CreateShader(vstr, fstr, CurrSig);
        CurrSig ^= T_Signature::FORWARD_PASS;

        CurrSig |= T_Signature::GBUFF_PASS;
        g_pBaseDriver->CreateShader(vstr, fstr, CurrSig);
        CurrSig ^= T_Signature::GBUFF_PASS;

        CurrSig |= T_Signature::SHADOW_MAP_PASS;
        g_pBaseDriver->CreateShader(vstr, fstr, CurrSig);
        CurrSig ^= T_Signature::SHADOW_MAP_PASS;

        CurrSig |= T_Signature::RADIAL_DEPTH_PASS;
        g_pBaseDriver->CreateShader(vstr, fstr, CurrSig);
  ...
  int	 RenderMesh::LoadTex(std::string p, xF::xMaterial *mat, Texture** tex) {
    ...
    *tex = g_pBaseDriver->GetTexture(id);
    ...
    unsigned int params = T_TEXT_BASIC_PARAMS::MIPMAPS;

    if (tiled)
      params |= T_TEXT_BASIC_PARAMS::TILED;
    else
      params |= T_TEXT_BASIC_PARAMS::CLAMP_TO_EDGE;
    ...
    //  long long Sig = -1;
     ...
     // ShaderBase *last = (ShaderBase*)32;
      ...
      for (std::size_t k = 0; k < it_MeshInfo->SubSets.size(); k++) {
        ...
        sub_info->IB->Set(*T8DeviceContext, 0, T_IB_FORMAT::R16);
