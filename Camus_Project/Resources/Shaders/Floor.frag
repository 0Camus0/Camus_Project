...
void main (void)
{
 	lowp  vec3 color = vec3(texture2D(sampler2d,varCoord));
	mediump  vec3 N = vec3(texture2D(normal2d,varCoord));
	N = (N*2.0) - 1.0;
	lowp  vec2 newTC = 0.5 * RasterPos.xy / RasterPos.w + vec2(0.5,0.5);	
	lowp  vec3 ReflectTerm = vec3(texture2D(reflect2d,newTC));
 	...
 	color*=totaldiffuse;
 	color+=totaldiffuse*ReflectTerm;
 	color+=totalspecular;
 	if(PCF)
		color*=ComputeShadowPCF();
	else
		color*=ComputeShadow();
